...
mod tests {
    ...
    fn selects_corrupted_deck_ekb_for_sony_0081_with_ff_leaf_id() {
        let ekb = Ekb::get_ekb_for_device(&[0xff; 8], SONY_VENDOR_ID, CORRUPTED_DECK_PRODUCT_ID);
    ...
    fn falls_back_to_open_source_ekb_for_normal_leaf_id() {
        let ekb = Ekb::get_ekb_for_device(
            &[0x01, 0x02, 0x03],
            SONY_VENDOR_ID,
    ...
    fn falls_back_to_open_source_ekb_for_other_devices_with_ff_leaf_id() {
        let ekb = Ekb::get_ekb_for_device(&[0xff; 8], SONY_VENDOR_ID, 0x0084);
